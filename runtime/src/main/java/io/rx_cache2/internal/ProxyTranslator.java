...
public final class ProxyTranslator {
    ...
    ConfigProvider processMethod(Method method, Object[] objectsMethod) {
        ...
        ConfigProvider configProvider = new ConfigProvider(prev.getProviderKey(),
                ...
                cacheMode(method, objectsMethod));
    ...
    private CacheMode cacheMode(Method method, Object[] objectsMethod) {
        CacheStrategy strategy = method.getAnnotation(CacheStrategy.class);
        if (strategy != null) {
            return strategy.value();
        }
        CacheMode cacheMode = getObjectFromMethodParam(method, CacheMode.class, objectsMethod);
        if (cacheMode != null) {
            return cacheMode;
        }
